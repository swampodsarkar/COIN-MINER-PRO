...
const TopUpModal: React.FC<TopUpModalProps> = ({ player, onClose, setPlayer }) => {
    const [activeTab, setActiveTab] = useState<'diamonds' | 'membership'>('diamonds');

    const onBuyGems = (amount: number) => {
        setPlayer(p => p ? { ...p, diamonds: p.diamonds + amount } : null);
    };

    const onBuyMembership = (type: 'weekly' | 'monthly', days: number) => {
        setPlayer(p => {
            ...
            return {
                ...
                activeMembership: {
                    ...
                    lastClaimedDailyDiamonds: new Date(now - 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Set to yesterday
                }
            };
        });
    };

    const TabButton: React.FC<{ tab: 'diamonds' | 'membership', label: string }> = ({ tab, label }) => (
        <button
            onClick={() => setActiveTab(tab)}
            className={`flex-1 py-3 text-lg font-bold transition-colors ${activeTab === tab ? 'text-orange-400 border-b-2 border-orange-400' : 'text-gray-400'}`}
        >
            {label}
        </button>
    );
        ...
        <Modal title="Shop" onClose={onClose}>
            <div className="w-full">
                <div className="flex border-b border-gray-700 mb-4">
                    <TabButton tab="diamonds" label="Diamonds" />
                    <TabButton tab="membership" label="Passes" />
                </div>
                {activeTab === 'diamonds' && (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-[60vh] overflow-y-auto p-1">
                        {gemPacks.map(pack => (
                            <div key={pack.gems} className={`relative bg-gray-900 border-2 rounded-lg p-3 text-center transition-transform hover:scale-105 ${pack.popular ? 'border-orange-500' : 'border-gray-700'}`}>
                                {pack.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-orange-500 text-black text-xs font-bold px-2 py-0.5 rounded-full">POPULAR</div>}
                ...
                {activeTab === 'membership' && (
                    <div className="space-y-4 p-1">
                         {membershipsData.map(sub => (
                             <div key={sub.type} className={`relative bg-gray-900 border-2 rounded-lg p-4 transition-transform hover:scale-105 ${sub.popular ? 'border-orange-500' : 'border-gray-700'}`}>
                                {sub.popular && <div className="absolute -top-3 left-4 bg-orange-500 text-black text-xs font-bold px-2 py-0.5 rounded-full">BEST VALUE</div>}
                                <div className="flex justify-between items-center">
                                    <div>
                                        <h4 className="text-lg font-bold text-orange-300">{sub.name}</h4>
                         ...
                         {player.activeMembership && (
                            <div className="text-center text-orange-400 p-2 bg-orange-500/10 rounded-lg mt-4">
